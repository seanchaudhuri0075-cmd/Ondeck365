...
SHOT = Path("/private/tmp/claude-501/-Users-gif025-Downloads-ondeck-pipeline/4bbac21b-8daa-486f-8c5d-924b6e198861/scratchpad/shots7")
    ...
    async with async_playwright() as p:
        ...
        for label, (w, h) in targets.items():
            ...
            for n in range(1, 35):
                ...
                if not box or box["height"] < 2:
                    continue          # collapsed on mobile by design
            ...
            if label == "mobile":
                audit = await pg.evaluate("""() => {
                  ...
                  for (let n=1;n<=34;n++){
                    ...
                    // A deliberately collapsed section (merged into the next
                    // slide) is not a short section — it has no screen at all.
                    const merged = h < 2;
                    if (!merged && h < window.innerHeight-2) short.push(n+':'+h+'px');

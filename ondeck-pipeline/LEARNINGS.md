...
- The blank-paragraph-spacer line-height fix (rule 22) lands ~86% of the measured target
  gap on the one deck it was checked against (49pt vs. 57pt), because it deliberately uses
  the browser's real line-height instead of a hand-tuned constant. The residual gap is
  suspected to be PowerPoint's own default paragraph spacing model doing something beyond
  pure line-height — worth a fixture-backed investigation once more decks are available to
  cross-check against, rather than closing it with a second curve-fit.

---

## 17. `EMU_PER_PT` must be 12700, not 9525 — 9525 is EMU-per-*pixel*

- **Symptom** — Every font-size and letter-spacing value rendered ~25% smaller than the
  source across an entire deck; shape *positions* looked correct throughout.
- **Root cause** — `EMU_PER_PT` was set to 9525, which is EMU-per-pixel at 96 DPI
  (914400/96), not EMU-per-point (914400/72=12700) — a units bug present in four files,
  one of which had a comment explicitly noting 9525 "looks wrong" and keeping it anyway
  for internal consistency. Shape x/y/w/h are used only as *ratios* against slide
  width/height (both computed with the same wrong constant), so the error canceled out
  there — but font-size and letter-spacing come from a genuinely different, already-correct
  unit (`sz`/`spc`, hundredths of a point, unrelated to EMU) divided by a wrongly-inflated
  slide width, with no matching error to cancel against.
- **Rule** — `EMU_PER_PT = 12700` everywhere. When changing a shared units constant, audit
  every consumer for whether it's used as a ratio (self-canceling, safe) or mixed with a
  differently-derived unit (not self-canceling, will be wrong). *Assertion:* a 48pt run in
  a fixture deck must occupy the same fraction of canvas width as `48/slide_width_pt`.
- **Proven by** — SHELFBEAUTY (headline measured at 25.8% of frame width vs. reference
  ~55%; declared 48pt, confirmed correct in isolation — the width discrepancy was entirely
  the units bug).

---

## 18. Font-family CSS values must use single quotes, never double

- **Symptom** — Every run's `font-family` was silently ignored; all text on every slide
  rendered in one hardcoded fallback font regardless of the run's declared typeface.
- **Root cause** — `font-family:"Helvetica Light"` was written into a `style="..."`
  attribute that is itself double-quoted. The browser's HTML parser treated the embedded
  `"` as closing the attribute early, garbling everything after it into bogus attributes.
- **Rule** — CSS font-family values embedded in an inline `style="..."` attribute use
  single quotes: `font-family:'Helvetica Light'`. *Assertion:* no double quote character
  appears inside the value of a double-quoted HTML attribute anywhere in generated markup.
- **Proven by** — SHELFBEAUTY (confirmed via `getComputedStyle` returning the page-level
  fallback font on every run; outerHTML inspection showed the corrupted attribute).

---

## 19. Compound typeface names encode weight/width — split them before matching

- **Symptom** — "Helvetica Light" resolved to a generic system default with different
  (wider) metrics than real Helvetica, throwing off text width and wrapping. Separately,
  "DIN Condensed" lost its condensed appearance once font-family resolution was fixed
  (regressed to a plain sans, since no font is literally installed under that exact name).
- **Root cause** — PPTX typefaces often bake weight/width into the family string itself
  rather than using a separate weight attribute. `fontconfig`/browsers usually only
  resolve a compound name if a font is installed under that *exact* string; the base name
  alone ("Helvetica") commonly has a real alias, but the compound string does not.
- **Rule** — Split a trailing weight keyword (Light/Bold/Medium/etc.) off the family name,
  apply it as a real numeric `font-weight`, and try the base name in the font stack (after
  first trying the literal compound name, in case it genuinely is installed). Detect
  "Condensed"/"Narrow" in the name and apply `font-stretch: condensed` separately, since
  that's a width axis, not a weight one. *Assertion:* the resolved font stack for a
  compound name always includes the weight-stripped base family as a fallback.
- **Proven by** — SHELFBEAUTY ("Helvetica Light" → real system alias once split;
  "DIN Condensed" → `font-stretch: condensed` applied even without the literal font).

---

## 20. Manual tab/space positioning needs real tab-stop math, and must not survive to mobile

- **Symptom** — A header manually right-shifted via 5 leading tab characters (plus literal
  padding spaces) rendered flush-left instead — `white-space: normal` collapses leading
  whitespace entirely. After preserving whitespace, the shift was still wrong until tab
  stops were computed from the deck's actual `defaultTextStyle/@defTabSz`, not the
  browser's arbitrary default tab-size. Separately, the same tab trick carried over to the
  mobile reflow verbatim, eating a chunk of a narrow column and causing early wraps.
- **Root cause** — Some decks position text via literal tab/space characters rather than
  real alignment properties — a manual hack that only makes sense against the specific
  wide desktop box it was tuned for.
- **Rule** — Preserve whitespace (`white-space: pre-wrap`) only on paragraphs whose runs
  actually contain a tab character (not blanket-applied — it also breaks `justify` on
  unrelated paragraphs sharing the same CSS rule). Compute `tab-size` from the deck's real
  `defTabSz` (read once per deck, EMU→pt via rule 17's constant), not the browser default.
  On mobile, strip leading tabs (and their padding spaces) from runs entirely — the
  positioning trick has no meaning in a narrow reflowed column. *Assertion:* `tab-size` in
  generated CSS is always derived from the source deck's `defaultTextStyle`, never left at
  the browser default; no mobile-reflow run contains a leading `\t`.
- **Proven by** — SHELFBEAUTY ("The Market"/"The Vision" headers).

---

## 21. `justify` must become `center` on mobile, not survive verbatim

- **Symptom** — A paragraph justified in its wide desktop box wrapped to 2–3 words per
  line in the narrow mobile column, and `justify` stretched the sparse inter-word gaps
  into visibly broken holes.
- **Root cause** — `text-align: justify` relies on many words per line to distribute
  extra space unnoticeably. A narrow reflowed column doesn't have that headroom — the same
  alignment value that was correct on desktop becomes actively wrong on mobile.
- **Rule** — On the mobile reflow only, remap `justify` → `center`. Desktop keeps the
  source-accurate `justify`. *Assertion:* no mobile-reflow paragraph has
  `text-align: justify` in generated CSS.
- **Proven by** — SHELFBEAUTY (cover slide subtitle).

---

## 22. Blank paragraphs are real manual spacers — give them real line-height, not a curve-fit height

- **Symptom** — Paragraphs separated by the source deck's blank-paragraph spacer lines
  rendered with visibly wrong gaps: first collapsed to ~0 (blank lines dropped entirely,
  no defRPr size to key off), then ~18pt (an attempt that collapsed consecutive blanks
  into one), then a hand-tuned `*1.43` constant that matched one reference image at ~57pt
  but has no principled basis.
- **Root cause** — Two distinct sub-bugs. (a) A blank `<a:p>` with zero runs and no
  `defRPr` has no size info in isolation — the fix is a documented, scoped fallback to the
  text frame's own body size (not a silent drop), applied narrowly (only when the blank
  paragraph carries bullet/indent `pPr`, to avoid guessing at ordinary trailing blanks that
  were correctly being ignored). (b) The actually-correct height for a blank spacer line is
  its real rendered line-height, the same mechanism every visible text line already uses —
  not an explicit pixel height computed from a hand-tuned multiplier chasing one screenshot
  (see rule 12's warning against exactly this pattern). Rendering the blank line as real
  text content (`&nbsp;`) at its real font-size and letting the browser's own line-height
  apply lands close to the measured target (~86% on the one deck checked) without a
  per-deck constant.
- **Rule** — Never silently drop a blank paragraph that carries pPr (bullet/indent or
  defRPr) — fall back to the frame's body size, scoped to avoid guessing at unrelated
  trailing blanks. Render blank spacer lines as real sized text content and let CSS
  line-height determine height; do not hand-tune a height multiplier against a screenshot.
  *Assertion:* no blank-paragraph spacer in generated CSS has an explicit `height`; its
  height is real font-size + inherited line-height.
- **Proven by** — SHELFBEAUTY ("The Market" body paragraphs; bulleted list spacer lines).

---

## 23. OOXML bullets are symbol-font codepoints — map them, don't render raw, and never use `inline-block` with negative `text-indent`

- **Symptom** — Bulleted list items rendered as plain paragraphs with no bullet marker at
  all and no per-item spacing (fell through to rule 3's "unrecognized shape structure"
  path entirely). After adding bullet parsing, the bullet glyph still didn't render —
  confirmed present in the DOM with correct color/size/position via computed styles, but
  invisible in the actual rendered pixels.
- **Root cause** — Two layered bugs. (a) `<a:buChar char="§">` with
  `<a:buFont typeface="Wingdings">` encodes a bullet glyph as a codepoint *in the Wingdings
  symbol font* — rendering that raw codepoint in a real Latin font shows the wrong
  character or nothing. (b) The hanging-indent hallmark hides a genuine Chromium
  rendering bug: an `inline-block` element with an explicit `width` as the very first
  content of a paragraph with negative `text-indent` fails to render at all — reproduced
  in isolation outside this codebase. A plain inline `<span>` (no `display:inline-block`,
  no explicit `width`) with the same `padding-left`/`text-indent` hanging-indent pattern
  renders correctly.
- **Rule** — Map known symbol-font bullet codepoints (Wingdings/Wingdings 2/3/Symbol) to
  real Unicode bullet characters via a small lookup table; default to a plain round bullet
  (•) for an unmapped codepoint in a known symbol font rather than showing the wrong glyph.
  Render the bullet as plain inline content (never `inline-block` + `width`) followed by
  the item text, using `padding-left`/`text-indent` for the hanging indent. *Assertion:*
  no bullet marker span in generated markup uses `display:inline-block` combined with a
  negative `text-indent` on its parent.
- **Proven by** — SHELFBEAUTY ("Retail Zones:" / "Skin Lab:" bulleted list).
